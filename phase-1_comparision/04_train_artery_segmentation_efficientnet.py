# 04_train_artery_segmentation_efficientnet.py
...
# [MODIFIED] Import the new EfficientNet-based model
from models_efficientnet import EfficientNetUnetModel
...
def main(args):
    ...
    model = model = EfficientNetUnetModel(num_classes=2)
    ...
    print("\n" + "="*50)
    print("      STARTING FINAL EVALUATION ON TEST SET (efficient-Net)")
    print("="*50)
    
    test_results = trainer.test(datamodule=datamodule, ckpt_path='best')
    
    print("\n" + "="*50)
    print("             TESTING COMPLETE (efficient-Net)")
    print("="*50)
    ...
    final_checkpoint_path = 'artery_backbone_efficientnet.pth'
    ...
    print(f"✅ Training complete. Best efficient-Net checkpoint at: {best_model_path}")
    print(f"✅ Final efficient-Net backbone for Phase 2 saved to: {final_checkpoint_path}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Train Phase 1 Artery Segmentation Model using efficient-Net-U-Net")
    ...
    parser.add_argument('--batch_size', type=int, default=4, help='efficient-Net is heavy; a small batch size is recommended')
