# 02_train_artery_segmentation_segformer.py
...
# [MODIFIED] Import the new SegFormer/MiT-based model
from models_segformer import SegFormerUnetModel
...
def main(args):
    ...
    # [MODIFIED] Instantiate the new SegFormerUnetModel
    model = SegFormerUnetModel(num_classes=2)
    task = ArterySegmentationTask(model, learning_rate=args.lr)

    # [MODIFIED] Changed paths to avoid overwriting previous experiments
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        dirpath='checkpoints_segformer',
        filename='artery-seg-segformer-{epoch:02d}-{val_dice_score:.4f}',
        save_top_k=1,
        verbose=True,
    ...
    trainer = pl.Trainer(
        ...
        logger=pl.loggers.TensorBoardLogger('logs/', name='artery_segmentation_segformer'),
    ...
    print("\n" + "="*50)
    print("      STARTING FINAL EVALUATION ON TEST SET (SegFormer-Unet)")
    print("="*50)
    
    test_results = trainer.test(datamodule=datamodule, ckpt_path='best')
    
    print("\n" + "="*50)
    print("             TESTING COMPLETE (SegFormer-Unet)")
    print("="*50)
    ...
    final_checkpoint_path = 'artery_backbone_segformer.pth'
    ...
    print(f"✅ Training complete. Best SegFormer-Unet checkpoint at: {best_model_path}")
    print(f"✅ Final SegFormer-Unet backbone for Phase 2 saved to: {final_checkpoint_path}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Train Phase 1 Artery Segmentation Model using a SegFormer-style U-Net")
    ...
    parser.add_argument('--batch_size', type=int, default=8, help='MiT encoders are efficient, you can likely use a batch size of 8')
