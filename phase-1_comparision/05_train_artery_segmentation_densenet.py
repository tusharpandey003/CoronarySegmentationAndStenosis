# 05_train_artery_segmentation_densenet.py
...
# [MODIFIED] Import the new DenseNet-based model
from models_densenet import DenseNetUnetModel
...
def main(args):
    ...
    model = DenseNetUnetModel(num_classes=2)
    task = ArterySegmentationTask(model, learning_rate=args.lr)

    # [MODIFIED] Changed paths for this specific experiment
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        dirpath='checkpoints_densenet',
        filename='artery-seg-densenet-{epoch:02d}-{val_dice_score:.4f}',
        save_top_k=1,
        verbose=True,
    ...
    trainer = pl.Trainer(
        ...
        logger=pl.loggers.TensorBoardLogger('logs/', name='artery_segmentation_densenet'),
    ...
    print("\n" + "="*50)
    print("      STARTING FINAL EVALUATION ON TEST SET (DenseNet121)")
    print("="*50)
    
    test_results = trainer.test(datamodule=datamodule, ckpt_path='best')
    
    print("\n" + "="*50)
    print("             TESTING COMPLETE (DenseNet121)")
    print("="*50)
    ...
    final_checkpoint_path = 'artery_backbone_densenet.pth'
    ...
    print(f"✅ Training complete. Best DenseNet121 checkpoint at: {best_model_path}")
    print(f"✅ Final DenseNet121 backbone for Phase 2 saved to: {final_checkpoint_path}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Train Phase 1 Artery Segmentation Model using DenseNet121-U-Net")
    ...
    parser.add_argument('--batch_size', type=int, default=4, help='DenseNet121 is heavy; a small batch size is recommended')
